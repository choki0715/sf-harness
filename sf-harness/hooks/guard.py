...
수업 포인트: 훅은 '설득되지 않는' 안전장치다.
  ...
  스킬에 "설비 정지는 직접 실행하지 마라"라고 써두면 대체로 지킨다.
  대체로. 진동이 CRIT 연속 12샘플이고 사용자가 "빨리 어떻게 좀 해봐"라고 재촉하면
  LLM 은 스스로를 설득할 수 있다. 훅은 프롬프트가 아니라 코드다. 협상 대상이 아니다.

  이 훅이 막는 것은 세 가지뿐이다. 전부 되돌리기 어렵거나 안전과 직결된다.
    1. 설비 정지 (sf-actuate stop)         — 가공 중인 제품 폐기, 재가동에 수십 분
    2. 원본 센서 로그 삭제·덮어쓰기 (signals/) — 사고 조사의 증거가 사라진다
    3. 한계값 수정 (config/thresholds.csv)  — 경보를 없애는 가장 쉬운 방법은 기준을 올리는 것이다.
                                               모델이 이 유혹에 빠지지 않게 아예 막는다

  규칙 1: 되돌릴 수 있는 일(작업지시 발행, 속도 저감)은 스킬에 맡기고,
          되돌릴 수 없는 일만 훅으로 막는다. 훅이 많으면 에이전트가 아무 일도 못 한다.
  규칙 2: 가드레일에는 범위를 준다. 이 훅은 `.sf-harness` 마커가 있는 디렉터리 안에서만 돈다.
          범위가 없으면 아무 저장소의 `rm signals.txt` 까지 막는다.

한계를 분명히 알고 쓴다: 훅은 셸 파서가 아니라 정규식이다.
변수 확장(`$CMD stop`), python -c 로 파일 열기, 별칭으로 우회할 수 있다.
...
# 명령의 '시작 위치'만 본다. 이게 없으면 `echo "sf-actuate stop 예시"` 같은
# 따옴표 안 문자열까지 명령으로 오인한다.
CMD_START = r"(?:\A|[;&|]|\n)\s*(?:sudo\s+)?"

# (패턴, 사람이 읽을 이유) — 되돌리기 어려운 것만 넣는다
DANGEROUS = [
    # sf-actuate [--옵션 값 ...] stop   — 서브커맨드 자리만 본다. --reason "stop" 같은 인자는 잡지 않는다
    (re.compile(CMD_START + r"(?:\S*/)?sf-actuate\s+(?:-\S+\s+\S+\s+)*(?:stop|emergency-stop)\b"),
     "설비 정지는 되돌릴 수 없다. 에이전트가 실행하지 않는다.\n"
     "권고와 근거(센서·값·한계)를 보고하고, 실행은 운영자가 터미널에서 한다."),
...
def in_scope(data: dict) -> bool:
    ...
    return bool(fp) and has_marker_above(os.path.dirname(fp))
...
def main() -> int:
    ...
    # 대상 디렉터리가 아니면 아무것도 하지 않는다. 여기가 범위 제한의 전부다.
    ...
    if PROTECTED_FILE.search(file_path.replace(os.sep, "/")):
